<template>
  <div id="wrapper">	
	...
	<div id="termometer">
		<div id="temperature" :style="`height:${style}`" :data-value="total"></div>
		<div id="graduations"></div>
	</div>
	
	<div id="playground" v-if="votation">		
		<div class="command" >{{votation}}</div>
	</div>
</div>
</template>
...
export default {
 name: "temperature",
 props:['total','command','votation'],
 computed:{
   max () {
     return this.votation ? this.command[this.votation].price : 100
   },
   style () {
    let total = this.votation ? this.total / this.max * 100 : 0
